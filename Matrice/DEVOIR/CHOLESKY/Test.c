...
void triangulaireinf(float**Bt, float*b, float**y, int dim); //B*y = b
void triangulairesup(float**Bt, float*y, float**x, int dim); //B*y = b
...
///Traitement
	...
	triangulairesup(Bt, y, &x, dim);	//RESOLUTION TRIANGLE SUPERIEURE
...
void triangulairesup(float**Bt, float*y, float**vx, int dim){
	float*x = newVect(dim);
...
void triangulaireinf(float**Bt, float*b, float**vy, int dim){
	float*y = newVect(dim);
	for(int i = 0; i < dim; i++){
		float sm = 0;
		for(int j=0; j <i ; j++){
